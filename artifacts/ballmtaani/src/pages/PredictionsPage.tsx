    ...
    <div className="max-w-5xl mx-auto px-4 py-8 md:py-12">
      <div className="mb-8 border-b border-white/10">
        ...
        <div className="flex gap-6">
          <button onClick={() => setActiveTab("make")} className={`pb-3 text-sm md:text-base font-black uppercase tracking-widest transition-colors relative ${activeTab === "make" ? "text-white" : "text-gray-500 hover:text-gray-300"}`}>
            Make Prediction
            {activeTab === "make" && <span className="absolute bottom-0 left-0 w-full h-1 bg-[#B30000] rounded-t"></span>}
          </button>
          <button onClick={() => setActiveTab("my")} className={`pb-3 text-sm md:text-base font-black uppercase tracking-widest transition-colors relative ${activeTab === "my" ? "text-white" : "text-gray-500 hover:text-gray-300"}`}>
            My Predictions
              ...
              <div key={fixture.id} className={`bg-[rgba(20,20,25,0.95)] rounded-xl border ${isSaved ? 'border-green-500/50' : 'border-[#B30000]/30'} border-l-4 ${isSaved ? 'border-l-green-500' : 'border-l-[#B30000]'} p-6 shadow-xl relative overflow-hidden transition-all`}>
                ...
                <div className="flex items-center justify-between mb-6">
                  <div className="flex flex-col items-center gap-2 w-1/4">
                    <TeamLogo logo={fixture.homeLogo} initial={fixture.homeInitial} color={fixture.homeColor} size="lg" shadow />
                  ...
                  <div className="flex flex-col items-center gap-2 w-1/4">
                    <TeamLogo logo={fixture.awayLogo} initial={fixture.awayInitial} color={fixture.awayColor} size="lg" shadow />
        ...
        <div className="space-y-6">
          {!isLoggedIn ? (
            <div className="bg-[#1B1B1B] border border-white/10 rounded-xl p-8 text-center max-w-xl mx-auto mt-12">
              ...
              <button onClick={openLoginModal} className="bg-[#B30000] hover:bg-red-800 text-white font-bold uppercase tracking-wider px-8 py-3 rounded transition-colors">
                Log In / Sign Up
              </button>
            </div>
          ) : (
            <div className="space-y-4">
              {[
                { label: "Premier League • Yesterday", teams: "Arsenal vs Chelsea", predicted: "2 - 1", actual: "2 - 1", result: "EXACT! +10 pts", color: "green" },
                { label: "Premier League • Sat",       teams: "Man City vs Liverpool", predicted: "2 - 0", actual: "3 - 1", result: "Correct Result +5 pts", color: "yellow" },
                { label: "Ligue 1 • Fri",              teams: "PSG vs Monaco",       predicted: "2 - 1", actual: "0 - 3", result: "Missed -0 pts", color: "red" },
              ].map((item, i) => (
                <div key={i} className="bg-[#1B1B1B] border border-white/5 rounded-xl p-5 shadow-lg flex flex-col md:flex-row items-center gap-4 md:gap-6 justify-between">
                  <div className="flex items-center gap-4 w-full md:w-auto">
                    <div className={`w-1.5 h-12 rounded-full ${item.color === "green" ? "bg-green-500" : item.color === "yellow" ? "bg-yellow-500" : "bg-red-500"}`}></div>
                    <div>
                      <span className="text-[10px] text-gray-500 uppercase font-bold tracking-wider block mb-1">{item.label}</span>
                      <div className="flex items-center gap-2 font-bold text-sm md:text-base">{item.teams.split(" vs ").map((t, j) => <span key={j}>{j > 0 ? <span className="text-gray-600 mx-1">vs</span> : null}{t}</span>)}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-8 w-full md:w-auto bg-[#0B0B0B] p-3 rounded-lg border border-white/5">
                    <div className="text-center"><span className="block text-[10px] text-gray-500 uppercase font-bold mb-1">Predicted</span><span className="font-black text-lg">{item.predicted}</span></div>
                    <div className="text-center"><span className="block text-[10px] text-gray-500 uppercase font-bold mb-1">Actual</span><span className="font-black text-lg">{item.actual}</span></div>
                  </div>
                  <div className={`${item.color === "green" ? "bg-green-500/10 border-green-500/30 text-green-500" : item.color === "yellow" ? "bg-yellow-500/10 border-yellow-500/30 text-yellow-500" : "bg-red-500/10 border-red-500/30 text-red-500"} border px-4 py-2 rounded font-black uppercase tracking-wider text-xs w-full md:w-auto text-center`}>
                    {item.result}
                  </div>
                </div>
              ))}
