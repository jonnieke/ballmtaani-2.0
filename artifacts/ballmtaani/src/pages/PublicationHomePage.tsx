...
export default function PublicationHomePage() {
  ...
  const rail = takeUnique(storyPool, usedStories, 2);
  const kenya = takeMatchingUnique(storyPool, usedStories, ["kenya", "harambee", "fkf", "gor mahia", "afc leopards", "tusker", "east africa"], 2);
  const epl = takeMatchingUnique(storyPool, usedStories, ["premier league", "arsenal", "chelsea", "liverpool", "manchester united", "man utd", "spurs"], 2);
  const analysis = takeMatchingUnique(storyPool, usedStories, ["analysis", "opinion", "tactical", "tactics", "preview", "breakdown"], 2);
  const africa = takeMatchingUnique(storyPool, usedStories, ["africa", "caf", "afcon", "east africa", "national team"], 2);
  const wire = takeUnique(wirePool, usedStories, 2);
