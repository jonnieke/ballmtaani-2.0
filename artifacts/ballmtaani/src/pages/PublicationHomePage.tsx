      ...
      <section id="mtaa-daily" className="bg-black">
        <div className="mx-auto max-w-7xl px-4 py-4 lg:py-6">
          <div className="grid gap-3 lg:grid-cols-[1.38fr_0.62fr]">
            ...
            <aside className="space-y-3">{rail.length ? rail.slice(0, 3).map((article) => <StoryCard key={article.slug || article.link || article.id} article={article} compact />) : [1,2,3].map((i) => <div key={i} className="h-[166px] animate-pulse border border-white/10 bg-white/5" />)}<Link href="/news" className="flex items-center justify-between border-b border-white/[0.1] px-1 py-3 text-[10px] font-black uppercase tracking-[0.22em] text-[#FFD700]/80 transition-colors hover:text-white">More top stories <ChevronRight className="h-4 w-4" /></Link></aside>
          </div>
        </div>
      </section>
