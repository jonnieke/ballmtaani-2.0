...
function StoryCard({ article, compact = false }: { article: NewsArticle; compact?: boolean }) {
  return <Link href={href(article)} className={`group block transition ${compact ? "grid grid-cols-[104px_1fr] gap-3 border-b border-white/[0.06] pb-3 hover:border-white/[0.12]" : "overflow-hidden border border-white/[0.05] bg-black/20 hover:border-white/[0.12] hover:bg-black/30"}`}>
    <div className={`${compact ? "aspect-[4/3] rounded-[1.1rem]" : "aspect-[16/10]"} relative overflow-hidden bg-black/40 ${compact ? "" : "rounded-[1.15rem]"}`}>
      ...
      <section id="mtaa-daily" className="bg-black">
        <div className="mx-auto max-w-7xl px-4 py-5 lg:py-7">
          <div className="grid gap-4 lg:grid-cols-[1.35fr_0.7fr]">
            {lead ? <Link href={href(lead)} className="group relative min-h-[460px] overflow-hidden bg-[#0b0b0b] ring-1 ring-white/[0.05] md:min-h-[540px]">
              ...
              <div className="relative z-10 flex h-full flex-col justify-end p-5 sm:p-6 lg:p-8">
                ...
                <h1 className="max-w-xl font-serif text-3xl leading-[0.93] text-white sm:text-4xl lg:text-5xl">{lead.title}</h1>
