...
function storyKey(article: NewsArticle) {
  ...
  const linkKey = [article.slug, article.link].filter(Boolean).join("|").toLowerCase();
  return [titleKey, linkKey].filter(Boolean).join("::");
}
...
function SectionTitle({ eyebrow, title, copy, href: to, action }: { eyebrow: string; title: string; copy?: string; href?: string; action?: string }) {
  return <div className="mb-3 flex items-end justify-between gap-3 border-b border-white/[0.06] pb-2.5"><div><p className="text-[10px] font-black uppercase tracking-[0.26em] text-[#B30000]">{eyebrow}</p><h2 className="mt-2 font-serif text-xl text-white md:text-2xl">{title}</h2>{copy && <p className="mt-2 max-w-2xl text-xs leading-6 text-white/45">{copy}</p>}</div>{to && action && <Link href={to} className="inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-[0.2em] text-white/50 hover:text-white">{action}<ChevronRight className="h-3.5 w-3.5" /></Link>}</div>;
}

function StoryCard({ article, compact = false }: { article: NewsArticle; compact?: boolean }) {
  return <Link href={href(article)} className={`group block transition ${compact ? "grid grid-cols-[96px_minmax(0,1fr)] gap-3 border-b border-white/[0.08] pb-3 hover:border-white/[0.14]" : "overflow-hidden border border-white/[0.04] bg-white/[0.015] hover:border-white/[0.1] hover:bg-white/[0.03]"}`}>
    <div className={`${compact ? "aspect-[4/3] rounded-[0.9rem]" : "aspect-[16/10]"} relative overflow-hidden bg-black/40 ${compact ? "" : "rounded-[0.95rem]"}`}>
      ...
      {!compact && <div className="absolute inset-0 bg-gradient-to-t from-black via-black/20 to-transparent" />}
    </div>
    <div className={compact ? "pt-0.5" : "p-3.5"}>
      ...
      <h3 className={`${compact ? "text-[0.92rem]" : "text-[1.02rem]"} font-black leading-snug text-white transition group-hover:text-[#FFD700]`}>{article.title}</h3>
