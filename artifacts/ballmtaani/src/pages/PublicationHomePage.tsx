...
function StoryCard({ article, compact = false }: { article: NewsArticle; compact?: boolean }) {
  return <Link href={href(article)} className={`group block transition ${compact ? "grid grid-cols-[88px_minmax(0,1fr)] gap-2.5 border-b border-white/[0.07] pb-3 hover:border-white/[0.12]" : "hover:translate-y-[-1px]"}`}>
    <div className={`${compact ? "aspect-[4/3] rounded-[0.85rem]" : "aspect-[16/10] rounded-[1rem]"} relative overflow-hidden bg-black/40`}>
