...
function SectionTitle({ eyebrow, title, copy, href: to, action }: { eyebrow: string; title: string; copy?: string; href?: string; action?: string }) {
  return <div className="mb-4 flex items-end justify-between gap-4 border-b border-white/8 pb-3"><div><p className="text-[10px] font-black uppercase tracking-[0.26em] text-[#B30000]">{eyebrow}</p><h2 className="mt-2 font-serif text-2xl text-white md:text-3xl">{title}</h2>{copy && <p className="mt-2 max-w-2xl text-sm leading-6 text-white/45">{copy}</p>}</div>{to && action && <Link href={to} className="inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-[0.2em] text-white/60 hover:text-white">{action}<ChevronRight className="h-3.5 w-3.5" /></Link>}</div>;
}

function StoryCard({ article, compact = false }: { article: NewsArticle; compact?: boolean }) {
  return <Link href={href(article)} className={`group block overflow-hidden border border-white/8 bg-[#0c0d11] transition hover:border-white/18 ${compact ? "grid grid-cols-[96px_1fr]" : ""}`}>
...
function FixtureCard({ match }: { match: HomepageMatch }) {
  ...
  return <div className="border border-white/8 bg-[#0d1016] px-4 py-4">
...
export default function PublicationHomePage() {
  ...
  const internal = useMemo(() => sortStories(news.filter((article) => article.isInternal)), [news]);
  const external = useMemo(() => sortStories(news.filter((article) => !article.isInternal)), [news]);
  const lead = internal[0] || news[0];
  const rail = [internal[1], internal[2], internal[3], news[1], news[2]].filter(Boolean) as NewsArticle[];
  const kenya = useMemo(() => pickStories(internal.length ? internal : news, ["kenya", "harambee", "fkf", "gor mahia", "afc leopards", "tusker", "east africa"], 4), [internal, news]);
  const epl = useMemo(() => pickStories(internal.length ? internal : news, ["premier league", "arsenal", "chelsea", "liverpool", "manchester united", "man utd", "spurs"], 4), [internal, news]);
  const analysis = useMemo(() => pickStories(internal.length ? internal : news, ["analysis", "opinion", "tactical", "tactics", "preview", "breakdown"], 4), [internal, news]);
  const africa = useMemo(() => pickStories(internal.length ? internal : news, ["africa", "caf", "afcon", "east africa", "national team"], 4), [internal, news]);
  const wire = external.slice(0, 3);
  ...
  return (
    <main className="bg-[#060606] text-white">
      ...
      <section id="mtaa-daily" className="border-b border-white/8 bg-black">
          ...
          <div className="grid gap-4 lg:grid-cols-[1.35fr_0.7fr]">
            {lead ? <Link href={href(lead)} className="group relative min-h-[540px] overflow-hidden border border-white/10 bg-[#0b0b0b]">
              ...
              <div className="relative z-10 flex h-full flex-col justify-end p-5 sm:p-6 lg:p-8">
                ...
                <p className="mt-4 max-w-lg text-base leading-7 text-white/75 sm:text-lg">{lead.description || "Read the full story, then move through fixtures, analysis and the live desk."}</p>
                <div className="mt-6 flex flex-wrap items-center gap-3"><span className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/5 px-4 py-2 text-[10px] font-black uppercase tracking-[0.2em] text-white/75">Read story <ArrowRight className="h-3.5 w-3.5" /></span><span className="text-[10px] uppercase tracking-[0.24em] text-white/35">BallMtaani original / real data</span></div>
              </div>
