...
function StoryCard({ article, compact = false }: { article: NewsArticle; compact?: boolean }) {
  return <Link href={href(article)} className={`group block overflow-hidden bg-[#0c0d11] ring-1 ring-white/[0.05] transition hover:ring-white/[0.1] ${compact ? "grid grid-cols-[96px_1fr]" : ""}`}>
...
export default function PublicationHomePage() {
  ...
  return (
    <main className="bg-[#060606] text-white">
      ...
      <section id="epl" className="bg-[#08090b]"><div className="mx-auto max-w-7xl px-4 py-6 lg:py-8">{SectionTitle({ eyebrow: "Premier League", title: "Premier League", href: "/news", action: "View all" })}<div className="grid gap-4 lg:grid-cols-[1.25fr_0.75fr]">{epl[0] && <StoryCard article={epl[0]} /> }<div className="grid gap-3">{epl.slice(1,4).map((article) => <StoryCard key={article.slug || article.link || article.id} article={article} compact />)}</div></div></div></section>

      <section id="analysis" className="bg-[#09090a]"><div className="mx-auto max-w-7xl px-4 py-6 lg:py-8">{SectionTitle({ eyebrow: "Analysis & Opinion", title: "Analysis / Opinion", href: "/articles", action: "Long reads" })}<div className="grid gap-4 lg:grid-cols-[1.15fr_0.85fr]">{analysis[0] ? <StoryCard article={analysis[0]} /> : <div className="min-h-[320px] animate-pulse border border-white/10 bg-white/5" />}<div className="grid gap-3">{analysis.slice(1,4).map((article) => <StoryCard key={article.slug || article.link || article.id} article={article} compact />)}</div></div></div></section>
