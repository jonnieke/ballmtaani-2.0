...
function storyKey(article: NewsArticle) {
  ...
  return [article.slug, article.link].filter(Boolean).join("|").toLowerCase();
}
...
function SectionTitle({ eyebrow, title, copy, href: to, action }: { eyebrow: string; title: string; copy?: string; href?: string; action?: string }) {
  return <div className="mb-3 flex items-end justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.26em] text-[#B30000]">{eyebrow}</p><h2 className="mt-2 font-serif text-xl text-white md:text-2xl">{title}</h2>{copy && <p className="mt-2 max-w-2xl text-xs leading-6 text-white/45">{copy}</p>}</div>{to && action && <Link href={to} className="inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-[0.2em] text-white/60 hover:text-white">{action}<ChevronRight className="h-3.5 w-3.5" /></Link>}</div>;
}

function StoryCard({ article, compact = false }: { article: NewsArticle; compact?: boolean }) {
  return <Link href={href(article)} className={`group block transition ${compact ? "grid grid-cols-[92px_1fr] gap-2 border-b border-white/[0.06] pb-2.5 hover:border-white/[0.12]" : "overflow-hidden border border-white/[0.05] bg-black/20 hover:border-white/[0.12] hover:bg-black/30"}`}>
    <div className={`${compact ? "aspect-[4/3] rounded-[0.95rem]" : "aspect-[16/10]"} relative overflow-hidden bg-black/40 ${compact ? "" : "rounded-[1rem]"}`}>
    ...
    <div className={compact ? "pt-0.5" : "p-4"}>
      ...
      {!compact && article.description && <p className="mt-2 line-clamp-2 text-xs leading-6 text-white/50">{article.description}</p>}
    </div>
  </Link>;
}
      ...
      <section id="mtaa-daily" className="bg-black">
        <div className="mx-auto max-w-7xl px-4 py-4 lg:py-6">
          <div className="grid gap-3 lg:grid-cols-[1.35fr_0.7fr]">
            {lead ? <Link href={href(lead)} className="group relative min-h-[430px] overflow-hidden bg-[#0b0b0b] ring-1 ring-white/[0.05] md:min-h-[500px]">
              ...
              <div className="relative z-10 flex h-full flex-col justify-end p-5 sm:p-6 lg:p-8">
                ...
                <h1 className="max-w-xl font-serif text-3xl leading-[0.92] text-white sm:text-[2.7rem] lg:text-[3.15rem]">{lead.title}</h1>
                <p className="mt-4 max-w-lg text-sm leading-6 text-white/78 sm:text-base sm:leading-7">{lead.description || "The front page story, told cleanly."}</p>
