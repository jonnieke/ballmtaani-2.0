...
import { FAN_ZONES } from "../data/mockData";
...
export default function FanZonesPage() {
  ...
  const zone = FAN_ZONES.find(z => z.id === activeZone);
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 py-8 md:py-12 animate-in fade-in slide-in-from-right-4 duration-300">
        ...
        <div className="rounded-2xl border border-white/10 overflow-hidden shadow-2xl mb-8 bg-[#1B1B1B]">
          {/* Header */}
          <div className="p-8 relative overflow-hidden" style={{ backgroundColor: zone.color }}>
            ...
            <div className="relative z-10 flex items-end gap-6">
              <div className="w-24 h-24 rounded-full bg-white flex items-center justify-center shadow-xl border-4 border-white/20 overflow-hidden shrink-0">
                {zone.logo ? (
                  <img src={zone.logo} alt={zone.team} className="w-full h-full object-contain p-2" />
                ) : (
                  <span className="text-4xl font-black" style={{ color: zone.color }}>{zone.team[0]}</span>
                )}
              </div>
              <div className="pb-1">
                <h1 className="text-3xl md:text-4xl font-black text-white uppercase tracking-widest mb-1">{zone.team} Fan Zone</h1>
                <span className="text-white/80 font-bold flex items-center gap-1.5 text-sm">
                  <Users className="w-4 h-4" /> {zone.members}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="p-6 md:p-8">
              ...
              <div className="flex gap-4">
                ...
                <div className="flex-1">
                  <textarea
                    ...
                    placeholder={`What's on your mind about ${zone.team}?`}
            ...
            <div className="space-y-4">
              ...
              {[
                { author: "GunnerKev",    time: "2h ago", text: zone.preview, likes: 24 },
                { author: "FootballFan99", time: "4h ago", text: `Honestly, ${zone.team} is looking sharp this season. Keep this up and we're winning it all.`, likes: 12 },
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {FAN_ZONES.map(zone => (
          <div key={zone.id} className="bg-[#1B1B1B] rounded-xl border border-white/5 shadow-xl relative overflow-hidden group hover:border-white/20 transition-all hover:-translate-y-1">
            ...
            <div className="h-16 w-full" style={{ backgroundColor: zone.color, opacity: 0.25 }}></div>
            ...
            <div className="p-5 relative">
              ...
              <div className="absolute -top-10 left-5 w-16 h-16 rounded-full bg-white flex items-center justify-center shadow-lg border-4 border-[#1B1B1B] overflow-hidden">
                {zone.logo ? (
                  <img src={zone.logo} alt={zone.team} className="w-full h-full object-contain p-1.5" />
                ) : (
                  <span className="text-2xl font-black" style={{ color: zone.color }}>{zone.team[0]}</span>
                )}
              </div>

              <div className="mt-6 mb-4">
                <h3 className="text-xl font-black uppercase tracking-widest">{zone.team}</h3>
                <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider flex items-center gap-1 mt-1">
                  <Users className="w-3 h-3" /> {zone.members}
                </span>
              </div>

              <div className="bg-[#0B0B0B] rounded-lg p-3 mb-5 border border-white/5 h-20">
                <p className="text-xs text-gray-400 italic line-clamp-2">"{zone.preview}"</p>
              </div>
