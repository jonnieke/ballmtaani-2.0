...
import { Trophy, Users, MessageSquare, ChevronRight, Zap, Radio, Calendar, ExternalLink, TrendingUp, Target, Swords, Flame } from "lucide-react";
import { useMatches, useUpcomingFixtures, useRecentMatches, useLeaderboard, useDebates, useProfile } from "../hooks/useData";
...
export default function HomePage() {
  ...
  // Featured debate — most-voted active debate
  const featuredDebate = debates[0] || null;
  const debateTotal = featuredDebate ? (featuredDebate.leftVotes + featuredDebate.rightVotes) || 1 : 1;
  const leftPct  = featuredDebate ? Math.round((featuredDebate.leftVotes / debateTotal) * 100) : 50;
  const rightPct = featuredDebate ? 100 - leftPct : 50;

  // Show stream in hero when it's live or upcoming (high-value hook)
  const showStreamInHero = !!(heroVideo?.isLive || heroVideo?.isUpcoming);
  ...
  return (
    <div className="pb-24">
      ...
      {/* ── 1. TICKER ── */}
      <HeroTicker articles={tickerArticles} matches={tickerMatches} />

      {/* ── 2. HERO ── */}
      <section className="relative overflow-hidden border-b border-white/8 bg-[#040508]">
        {heroImages.map((src, i) => (
          <img key={src} src={src} alt="" decoding="async" loading={i === 0 ? "eager" : "lazy"}
          ...
          <div className="grid grid-cols-1 items-center gap-8 lg:grid-cols-2">
            ...
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.65 }}>
              ...
              <h1 className="mb-3 text-4xl font-black leading-[0.88] tracking-tight text-white md:text-5xl lg:text-6xl">
                {isMatchLive && liveMatch ? (
                  <>
                    {liveMatch.home} {liveMatch.homeScore}
                    <span className="text-white/30">–</span>
                    {liveMatch.awayScore} {liveMatch.away}<br />
                    <span className="text-[#B30000]">{liveMatch.minute}′. Your move.</span>
                  </>
                ) : wc26.isLive ? (
                  <>
                    The World Cup<br />
              ...
              <div className="flex flex-wrap gap-3">
                {!isLoggedIn ? (
                  ...
                  <>
                    {isMatchLive && liveMatch ? (
                      <Link href={`/live-center/${liveMatch.id}`}
            ...
            {/* Right — stream when live/upcoming, else match cards */}
            <motion.div initial={{ opacity: 0, scale: 0.94 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.65, delay: 0.15 }}
              className="flex flex-col gap-3">
              {showStreamInHero && heroVideo ? (
                <>
                  {/* Live stream card */}
                  <div className="overflow-hidden rounded-2xl border border-[#B30000]/40 bg-black shadow-[0_0_32px_rgba(179,0,0,0.2)]">
                    <div className="flex items-center justify-between border-b border-[#B30000]/18 px-4 py-2.5">
                      <div className="flex items-center gap-2">
                      ...
                      <a href={SPORTYTV_CHANNEL_URL} target="_blank" rel="noopener noreferrer"
                        className="text-[10px] font-black text-white/30 hover:text-white transition-colors">All →</a>
                    </div>
                    <div className="aspect-video w-full">
                      <YouTubePlayer key={heroVideo.id} videoId={heroVideo.id} title={heroVideo.title} className="h-full w-full" onUnavailable={handleVideoUnavailable} />
                    </div>
                    {heroVideo.title && <p className="truncate border-t border-white/8 px-4 py-2 text-[11px] text-white/50">{heroVideo.title}</p>}
                  </div>
                  {/* Compact live match card below stream */}
            ...
            <div className="flex justify-center gap-1.5 pb-4 pt-2">
              {heroImages.map((_, i) => (
                <button key={i} onClick={() => setHeroIndex(i)}
                  className={`rounded-full transition-all duration-300 ${i === heroIndex ? "w-5 h-1.5 bg-[#FFD700]" : "w-1.5 h-1.5 bg-white/25 hover:bg-white/50"}`}
      ...
      {isLoggedIn && (
        <section className="border-b border-white/6 bg-[#07080d] py-3">
          <div className="mx-auto max-w-6xl px-4">
            <div className="flex items-center gap-2 overflow-x-auto [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              {/* Welcome chip */}
              <div className="flex shrink-0 items-center gap-2 rounded-lg border border-white/8 bg-white/[0.03] px-3 py-2">
                <span className="text-[10px] font-black text-white/60">
                  {profile?.username && !String(profile.username).startsWith("Fan_")
                    ? `Hey, ${String(profile.username).split(" ")[0]} 👋`
                    : "Welcome back 👋"}
                </span>
              </div>
              {/* Points */}
              <div className="flex shrink-0 items-center gap-1.5 rounded-lg border border-[#FFD700]/20 bg-[#FFD700]/6 px-3 py-2">
                <span className="text-[10px] font-black text-[#FFD700]">{(profile?.points || 0).toLocaleString()} MTC</span>
              </div>
              {/* Streak */}
              {(profile?.streak || 0) > 0 && (
                <div className="flex shrink-0 items-center gap-1.5 rounded-lg border border-orange-500/20 bg-orange-500/6 px-3 py-2">
                  <Flame className="h-3 w-3 text-orange-400" />
                  <span className="text-[10px] font-black text-orange-400">{profile?.streak}d streak</span>
                </div>
              )}
              {/* Divider */}
              ...
              {/* CTAs */}
              <Link href="/predictions"
                className="shrink-0 rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-[10px] font-black uppercase tracking-widest text-white/60 transition-all hover:bg-white/10 hover:text-white">
                My Calls
              </Link>
              ...
              <Link href="/rivalries"
                className="shrink-0 rounded-lg border border-[#B30000]/25 bg-[#B30000]/8 px-3 py-2 text-[10px] font-black uppercase tracking-widest text-[#ff6b6b] transition-all hover:bg-[#B30000]/15">
                Challenge a Rival →
              </Link>
            </div>
          </div>
      ...
      {/* ── 6. PREDICTION CTA ── */}
      {nextFixture && (
        <section className="border-b border-white/6 bg-[#08050d] py-5">
          <div className="mx-auto max-w-6xl px-4">
            <div className="flex flex-col items-center gap-4 rounded-2xl border border-[#B30000]/25 bg-[#0d0608]/80 px-5 py-5 sm:flex-row sm:items-center">
              {/* Teams */}
              <div className="flex flex-1 items-center gap-3">
                <TeamLogo logo={nextFixture.homeLogo} initial={nextFixture.homeInitial} color={nextFixture.homeColor || "#182333"} size="md" />
                <div className="text-center">
                  <div className="text-[10px] font-black uppercase tracking-widest text-white/30">
                    {nextFixture.league || "Upcoming"}
                  </div>
                  <div className="text-lg font-black text-white">
                    {nextFixture.home} <span className="text-white/25">vs</span> {nextFixture.away}
                  </div>
                  {nextFixture.time && (
                    <div className="text-[11px] font-bold text-[#FFD700]">
                      {nextFixture.date ? `${nextFixture.date} · ` : ""}{nextFixture.time}
                    </div>
                  )}
                </div>
                <TeamLogo logo={nextFixture.awayLogo} initial={nextFixture.awayInitial} color={nextFixture.awayColor || "#182333"} size="md" />
              </div>
              {/* CTA */}
              <Link href="/predictions"
                className="shrink-0 rounded-xl bg-[#B30000] px-6 py-3 text-sm font-black uppercase tracking-[0.12em] text-white shadow-[0_0_20px_rgba(179,0,0,0.35)] transition-all hover:shadow-[0_0_30px_rgba(179,0,0,0.55)] active:scale-95">
                Make Your Call →
              </Link>
            </div>
          </div>
        ...
        <section className="border-b border-white/6 bg-[#04060a] py-8">
          <div className="mx-auto max-w-6xl px-4">
            <div className="mb-4 flex items-center justify-between">
              <div>
                ...
                <p className="mt-0.5 text-[10px] font-semibold uppercase tracking-widest text-white/28">
                  {Number(featuredDebate.totalVotes?.toString().replace(/,/g, "") || 0).toLocaleString()} fans already voted
                </p>
              </div>
              <Link href="/debates" className="text-[10px] font-black uppercase tracking-widest text-[#B30000] hover:underline">
                All Debates →
              </Link>
            </div>
            ...
            <div className="overflow-hidden rounded-2xl border border-white/10 bg-[#0a0c12]">
              <div className="p-5">
                <p className="mb-5 text-base font-black leading-snug text-white md:text-lg">{featuredDebate.title}</p>
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-black text-white truncate">{featuredDebate.left}</span>
                      <span className="text-sm font-black text-[#FFD700] ml-2">{leftPct}%</span>
                    </div>
                    <div className="h-2.5 overflow-hidden rounded-full bg-white/8">
                      <div className="h-full rounded-full bg-[#FFD700] transition-all duration-700" style={{ width: `${leftPct}%` }} />
                    </div>
                  </div>
                  {/* Right option */}
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center justify-between">
                      <span className="text-[#B30000] font-black text-sm ml-auto">{rightPct}%</span>
                      <span className="text-sm font-black text-white truncate ml-2 text-right">{featuredDebate.right}</span>
                    </div>
                    <div className="h-2.5 overflow-hidden rounded-full bg-white/8">
                      <div className="h-full rounded-full bg-[#B30000] transition-all duration-700 ml-auto" style={{ width: `${rightPct}%` }} />
                    </div>
                  </div>
                </div>
                ...
                <div className="mb-4 h-1.5 overflow-hidden rounded-full bg-white/5">
                  <div className="h-full bg-gradient-to-r from-[#FFD700] to-[#B30000]" />
                </div>

                <Link href="/debates"
                  className="block w-full rounded-xl border border-white/12 bg-white/5 py-3 text-center text-xs font-black uppercase tracking-[0.14em] text-white transition-all hover:bg-white/10 active:scale-[0.98]">
                  Add Your Take →
                </Link>
              </div>
            </div>
      ...
      <section className="border-b border-white/6 bg-[#05070b] py-10">
        <div className="mx-auto max-w-6xl px-4">
          ...
          {/* BallMtaani articles horizontal scroller */}
          {news.filter(a => a.isInternal).length > 0 && (
            <div className="mb-6">
              <div className="mb-3 flex items-center justify-between">
                <span className="flex items-center gap-2 text-[9px] font-black uppercase tracking-[0.28em] text-white/35">
                  <span className="h-3 w-1 rounded-full bg-[#B30000]" /> Latest from BallMtaani
                </span>
                <div className="flex items-center gap-1.5">
                  <button onClick={() => scrollArticles("left")}
                    className="flex h-7 w-7 items-center justify-center rounded-full border border-white/10 bg-white/5 text-white/40 transition-all hover:bg-white/10 hover:text-white active:scale-95">
                ...
                {news.filter(a => a.isInternal).slice(0, 5).map((article, i) => (
                  <Link key={article.id} href={`/article/${article.slug}`}
                    ...
                    <div className="relative h-full w-24 shrink-0 overflow-hidden sm:w-28">
                      ...
                      <div className="absolute left-1.5 top-1.5 flex h-5 w-5 items-center justify-center rounded-full bg-[#B30000] text-[9px] font-black text-white">{i + 1}</div>
                    </div>
                    <div className="flex min-w-0 flex-1 flex-col justify-between p-3">
                      <div>
                        {article.isWC26 && <span className="mb-1 inline-block rounded bg-[#FFD700]/15 px-1.5 py-0.5 text-[7px] font-black uppercase tracking-wider text-[#FFD700]">WC26</span>}
                        <p className="line-clamp-3 text-[12px] font-black leading-snug text-white group-hover:text-white/90">{article.title}</p>
                      </div>
          ...
          {/* SportyTV strip — always accessible for latest video */}
          {!showStreamInHero && heroVideo && (
            <div className="mt-6 overflow-hidden rounded-2xl border border-white/10 bg-black">
              <div className="flex items-center justify-between border-b border-white/8 px-4 py-2.5">
                <div className="flex min-w-0 items-center gap-2">
                  ...
                  <span className={`truncate text-[10px] font-black uppercase tracking-[0.18em] ${heroVideo.isLive ? "text-[#B30000]" : heroVideo.isUpcoming ? "text-[#FFD700]/90" : "text-white/55"}`}>
                    {heroVideo.isLive ? "Live Match — SportyTV" : heroVideo.isUpcoming && heroVideo.startsAt ? `Next Stream · ${formatStartTime(heroVideo.startsAt)}` : "Watch — SportyTV"}
                  </span>
                </div>
              ...
              {heroVideo.title && <p className="truncate border-t border-white/8 px-4 py-2.5 text-[11px] font-bold text-white/55">{heroVideo.title}</p>}
            </div>
          )}
        ...
        <div className="mx-auto max-w-6xl px-4">
          ...
          <div className="grid grid-cols-2 gap-2 lg:grid-cols-4 lg:gap-3">
            {[
              ...
              { href: "/ai-fan-zone",     img: "/funzone/mchambuzi.jpg",    label: "AI Fan Zone",  sub: "Quiz, card, genius take — all AI",  cta: "Enter Zone" },
      ...
      <section className="relative overflow-hidden border-b border-white/6 bg-[#030507] py-12">
        ...
        <div className="relative z-10 mx-auto max-w-6xl px-4">
          ...
          {(() => {
            const CAF_NATIONS = [
              { code: "ma", name: "Morocco",      group: "D", note: "Qatar semi-finalists. Africa's most complete squad.",   color: "#C1272D" },
              { code: "sn", name: "Senegal",       group: "H", note: "AFCON champions. Mané's farewell tournament?",         color: "#00853F" },
              { code: "eg", name: "Egypt",         group: "D", note: "Salah's biggest stage. Pharaohs hunting history.",     color: "#CE1126" },
              { code: "ng", name: "Nigeria",       group: "F", note: "Super Eagles — dangerous, hungry, unpredictable.",    color: "#008751" },
              { code: "cm", name: "Cameroon",      group: "F", note: "Indomitable Lions. Built to upset big nations.",      color: "#007A5E" },
              { code: "za", name: "South Africa",  group: "A", note: "First World Cup since 2010. Back on the big stage.",  color: "#007A4D" },
              { code: "ci", name: "Côte d'Ivoire", group: "G", note: "AFCON holders. The Elephants charge again.",          color: "#F77F00" },
              { code: "ml", name: "Mali",          group: "E", note: "Eagles soaring. Tactical, disciplined, dangerous.",   color: "#14B53A" },
              { code: "tn", name: "Tunisia",       group: "B", note: "Eagles of Carthage facing European giants.",          color: "#E70013" },
            ];
            return (
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {CAF_NATIONS.map(nation => (
                  <Link key={nation.name} href="/world-cup-2026"
                    ...
                    <div className="p-4">
                      <div className="flex items-start gap-3">
                        <div className="h-11 w-16 shrink-0 overflow-hidden rounded border border-white/10 bg-black/30">
                          <img src={`https://media.api-sports.io/flags/${nation.code}.svg`} alt={nation.name}
                      ...
                      <div className="mt-3 flex items-center justify-between rounded-lg border border-white/6 bg-black/30 px-3 py-2">
                        <span className="text-[10px] font-bold text-white/50">Group Stage · WC26</span>
                        <span className="text-[10px] font-black uppercase tracking-widest text-[#FFD700] group-hover:underline">Track →</span>
                      </div>
                    </div>
                  </Link>
                ))}
      ...
      <section className="border-b border-white/6 bg-[#04060a] py-10">
        <div className="mx-auto max-w-6xl px-4">
          <div className="mb-6">
            <h2 className="text-lg font-black uppercase tracking-widest text-white">Challenge. <span className="text-[#B30000]">Debate.</span> Win.</h2>
            <p className="mt-0.5 text-[10px] font-semibold uppercase tracking-widest text-white/28">Fan Duels · Debates · Elite Fan Zones</p>
          </div>
